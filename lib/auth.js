 ...
 */
async function loginWithBrowser({ apiUrl, useBrowser = true, onEvent = () => {} } = {}) {
  ...
  try {
    ...
    if (loopback) {
      // Lăsăm browserului o clipă să încarce pagina de final înainte să închidem ascultătorul.
      setTimeout(() => { try { loopback.server.close(); } catch {} }, 2000).unref?.();
    }
  }
}
